...
Devanagari convert_FixedPosition(Unicode &var,int position)
{
	 ...
	 */
...

		{
			 ...
			 */
			/*
			 *Some other exceptions are handled here.
			 */
			if((var+1)!=EOU)
			{
				if(numerical_value[0]==0x30&&numerical_value[1]==0x41)
				{
					dev=db.return_OtherPreeti(3);
				}
				else if(numerical_value[0]==0x30&&numerical_value[1]==0x42)
				{
					dev=db.return_OtherPreeti(4);
				}
				else
				{
					dev=db.return_FullPreeti(numerical_value[0]);
